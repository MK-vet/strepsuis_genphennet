# User Guide for StrepSuis-GenPhenNet

Network-based integration of genomic and phenotypic data with statistical analysis
...
StrepSuis-GenPhenNet is a professional bioinformatics tool for analyzing bacterial genomic data. This guide provides step-by-step instructions for using the tool across three different deployment methods.
...
1. Pull the Docker image (when published):
   ...
   docker build -t strepsuis-genphennet:latest .
   ```

2. Test the container:
   ```bash
   docker run --rm strepsuis-genphennet:latest --help
   ```

3. Run with your data:
   ```bash
   docker run -v $(pwd)/data:/data -v $(pwd)/output:/output \
       strepsuis-genphennet:latest --data-dir /data --output /output
   ```
...
strepsuis-genphennet --data-dir . --output ./results
...
```bash
strepsuis-genphennet --data-dir examples/ --output test_output/
```
...
**File:** `Network_Analysis_report.html`
...
**File:** `Network_Analysis_Report_<timestamp>.xlsx`
...
├── Network_Analysis_Report_20250114_120000.xlsx
└── png_charts/
...
```bash
# Using example data
strepsuis-genphennet --data-dir examples/ --output results/
...
for dataset in data_*/; do
  ...
  strepsuis-genphennet --data-dir "$dataset" --output "$output"
done
```
...
#### Issue: "ModuleNotFoundError: No module named 'strepsuis_genphennet'"

**Solution:** Reinstall the package.

```bash
pip uninstall -y strepsuis_genphennet
pip install git+https://github.com/MK-vet/strepsuis-genphennet.git
```
...
docker run -v $(pwd)/data:/data strepsuis-genphennet:latest --data-dir /data --output /output
```
...
3. Search existing [GitHub Issues](https://github.com/MK-vet/strepsuis-genphennet/issues)
...
- **Source Code:** [GitHub Repository](https://github.com/MK-vet/strepsuis-genphennet)
- **Issues & Support:** [GitHub Issues](https://github.com/MK-vet/strepsuis-genphennet/issues)
...
**Need help?** Open an issue on [GitHub](https://github.com/MK-vet/strepsuis-genphennet/issues) or refer to the troubleshooting section above.
