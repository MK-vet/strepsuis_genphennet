...
def test_reproducibility(mini_dataset):
    ...
    # Run analysis twice
    analyzer1 = NetworkAnalyzer(config=config)
    analyzer2 = NetworkAnalyzer(config=config)
...
def test_configuration_impact(mini_dataset):
    ...
    analyzer_low = NetworkAnalyzer(config=config_low)
    analyzer_high = NetworkAnalyzer(config=config_high)
